            ...
            with lmdb_env.begin() as txn:
                for key, value in txn.cursor():
                    ...
                    if 'mask' in value:
                        ...
                        sem = None
                    cam_idx_mat = np.concatenate([np.ones((len(sem), 1, 256, 256)) * i for i in range(sem.shape[1])], axis=1)
                    assert cam_idx_mat.shape == sem.shape
                    ...
                    for t in range(num_steps):
                        ...
                        if sem is not None:
                            ...
                            t_imgidx = cam_idx_mat[t].reshape(-1)[in_mask]
                        #don't need voxelize
                        t_pc, mask = voxelize_pcd(t_pc, voxel_size=args.voxel_size)
                        t_rgb = t_rgb[mask]
                        if sem is not None:
                            t_sem = t_sem[mask]
                            t_imgidx = t_imgidx[mask]
